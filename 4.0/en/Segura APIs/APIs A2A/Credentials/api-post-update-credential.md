...
* `identifier` - string - required for updating the credential’s `username` - Unique string defined by the user or by Segura for identifying the credential.

Note: this value is created in [POST | Create credential ](../../../../../v3-33/docs/api-post-create-credential/)and is obtained in the response to the [GET | List all credentials](../../../../../v3-33/docs/api-get-list-all-credentials/) request.
...
Note: the username parameter must be sent when updating the credential, even if its value is an empty string. (`''`). In such case, it’ll be automatically changed to the default value `usr`.
...
| Condition             | ->                                                                                 | Result                                                     |                                                                                                   |
| --------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Existing `identifier` |                                                                                    | The credential will be updated with the other values sent. |                                                                                                   |
| New `identifier`      | The system will search for the credential by their `hostname`, `ip` and `username` | Credential found                                           | The credential will be updated with the other values sent, and its `identifier` will be changed.. |
...
\
Possible cause: forbidden characters were used when updating the credential’s `identifier`.
